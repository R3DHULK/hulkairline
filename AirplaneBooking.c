...
struct hulk_airline
{
	...
	int seat_num;
	char email[15];
	struct hulk_airline *following;
}
*begin, *stream;
struct hulk_airline *dummy;
...
void main()
{
	...
	do
	{
		...
		printf("\n\n\t\t ********************************************************************");
		printf("\n\t\t                   welcome to Sumalya's airline system                   ");
		printf("\n\t\t   *******************************************************************");
...
void reserve(int x)
{
	...
	{
		// first user
		begin = stream = (struct hulk_airline*)malloc(sizeof(struct hulk_airline));
		details();
	...
	else
	{
		...
		stream->following = (struct hulk_airline *)malloc(sizeof(struct hulk_airline));
		stream = stream->following;
		details();
		stream->following = NULL;
...
void savefile()
{
	FILE *fpointer = fopen("hulk records", "w");
	if (!fpointer)
	{
	...
	printf("\n\n\t Details have been saved to a file (hulk records)");
	fclose(fpointer);
}
//********************************GOOD LUCK HULK***************************************
